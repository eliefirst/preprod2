...
class Category extends AbstractEntity
{
    ...
    protected function _getDisplayModeField($rowDisplayMode)
    {
        ...
        foreach ($this->_displayMode->getAllOptions() as $displayMode) {
            $displayModes[] = $displayMode['value'];
        }
        if (!in_array($rowDisplayMode, $displayModes, true)) {
        ...
        if ($rowSku) {
            ...
            foreach ($productSku as $sku) {
                if ($product->getIdBySku($sku)) {
                    $productIds[$product->getIdBySku($sku)] = '1';
                }
            }
     ...
     * @param \Magento\Catalog\Model\ResourceModel\Category\Collection|\Magento\Catalog\Model\Category[] $children
     * @param int $rowNum
     *
     * @return string
     * @throws LocalizedException
    ...
    /**
     * @param \Magento\Catalog\Model\Category $updateCategory
